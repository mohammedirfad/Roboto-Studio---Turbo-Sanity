...
// Query: *[_type == "category"] | order(title asc) {    _id,    title,    "slug": slug.current,    color  }
...
// Source: ../../packages/sanity/src/query.ts
// Variable: queryBlogCategoryPageBlogsCount
// Query: count(*[_type == "blog" && (seoHideFromLists != true) && $categorySlug in categories[]->slug.current])
export type QueryBlogCategoryPageBlogsCountResult = number;

// Source: ../../packages/sanity/src/query.ts
// Variable: queryBlogCategoryPageBlogs
// Query: *[_type == "blog" && (seoHideFromLists != true) && $categorySlug in categories[]->slug.current] | order(orderRank asc) [$start...$end]{      _type,  _id,  title,  description,  "slug":slug.current,  orderRank,    image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  },  publishedAt,  pokemon,    authors[0]->{    _id,    name,    position,      image {      "id": asset._ref,  "preview": asset->metadata.lqip,  "alt": coalesce(    alt,    asset->altText,    caption,    asset->originalFilename,    "untitled"  ),  hotspot {    x,    y  },  crop {    bottom,    left,    right,    top  }  }  },  "categories": categories[]->{    _id,    title,    "slug": slug.current,    color  }  }
export type QueryBlogCategoryPageBlogsResult = Array<{
  _type: "blog";
  _id: string;
  title: string;
  description: string | null;
...
// Source: ../../packages/sanity/src/query.ts
// Variable: queryCategoryData
// Query: *[_type == "category" && slug.current == $categorySlug][0]{    _id,    title,    "slug": slug.current,    color,    description  }
export type QueryCategoryDataResult = {
  _id: string;
  title: string;
  slug: string | null;
  color:
